...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-2xl font-bold text-gray-900">Create New Store</h2>
            <p class="mt-1 text-sm text-gray-600">Add a new store to your business</p>
        </div>
    ...
    <!-- Create Form -->
    <x-card>
        <form action="{{ route('stores.store') }}" method="POST" enctype="multipart/form-data" x-data="storeForm()">
            @csrf
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            >
                                @foreach($timezones as $timezone)
                                    <option value="{{ $timezone }}" {{ old('timezone', 'Asia/Jakarta') === $timezone ? 'selected' : '' }}>
                                        {{ $timezone }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            >
                                @foreach($currencies as $code => $name)
                                    <option value="{{ $code }}" {{ old('currency', 'IDR') === $code ? 'selected' : '' }}>
                                        {{ $name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="flex items-center pt-7">
                            <x-form.checkbox
                                ...
                                :checked="old('tax_included', false)"
                            />
                        </div>
                        ...
                        <div>
                            ...
                            >
                                @foreach($roundingMethods as $value => $label)
                                    <option value="{{ $value }}" {{ old('rounding_method', 'none') === $value ? 'selected' : '' }}>
                                        {{ $label }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <textarea
                                ...
                                class="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-indigo-500 focus:border-indigo-500"
                            >{{ old('receipt_footer', 'Terima kasih atas kunjungan Anda!') }}</textarea>
...
@php
    ...
    $breadcrumb = [
        ...
        ['label' => 'Create', 'url' => route('stores.create')]
    ];
@endphp

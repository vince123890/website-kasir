<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Edit Stock Adjustment') }} - {{ $adjustment->adjustment_number }}
        </h2>
    </x-slot>
                    ...
                    <form method="POST" action="{{ route('inventory.adjustments.update', $adjustment->id) }}">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                            <div>
                                ...
                                <select name="store_id" id="store_id" required class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                    ...
                                    @foreach($stores as $store)
                                        <option value="{{ $store->id }}" {{ (old('store_id') ?? $adjustment->store_id) == $store->id ? 'selected' : '' }}>
                                            {{ $store->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <input type="date" name="adjustment_date" id="adjustment_date" value="{{ old('adjustment_date', $adjustment->adjustment_date->format('Y-m-d')) }}" max="{{ now()->format('Y-m-d') }}" required class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                            <div>
                                ...
                                <select name="product_id" id="product_id" required class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                    ...
                                    @foreach($products as $product)
                                        <option value="{{ $product->id }}" {{ (old('product_id') ?? $adjustment->product_id) == $product->id ? 'selected' : '' }}>
                            ...
                            <div>
                                ...
                                <select name="type" id="type" required class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                    ...
                                    <option value="add" {{ (old('type') ?? $adjustment->type) === 'add' ? 'selected' : '' }}>Add Stock</option>
                                    <option value="reduce" {{ (old('type') ?? $adjustment->type) === 'reduce' ? 'selected' : '' }}>Reduce Stock</option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                            <div>
                                ...
                                <input type="number" name="quantity" id="quantity" value="{{ old('quantity', $adjustment->quantity) }}" min="1" required class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                            </div>
                        ...
                        <div class="mb-6">
                            ...
                            <textarea name="notes" id="notes" rows="3" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">{{ old('notes', $adjustment->notes) }}</textarea>
                        </div>

                        <div class="flex justify-end space-x-3">
                            <a href="{{ route('inventory.adjustments.show', $adjustment->id) }}" class="px-4 py-2 bg-gray-300 text-gray-700 rounded-md hover:bg-gray-400">
                                Cancel
                            </a>

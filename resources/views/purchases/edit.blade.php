...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-600">Edit purchase order: {{ $purchaseOrder->po_number }}</p>
        </div>
    ...
    <!-- Edit Form -->
    <form action="{{ route('purchases.update', $purchaseOrder->id) }}" method="POST" x-data="purchaseOrderForm()">
        @csrf
        @method('PUT')
                ...
                <div class="space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="date" name="order_date" required value="{{ old('order_date', $purchaseOrder->order_date->format('Y-m-d')) }}" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-indigo-500 focus:border-indigo-500">
                            @error('order_date')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="date" name="expected_delivery_date" required value="{{ old('expected_delivery_date', $purchaseOrder->expected_delivery_date->format('Y-m-d')) }}" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-indigo-500 focus:border-indigo-500">
                    ...
                    <div>
                        ...
                        <textarea name="notes" rows="3" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-indigo-500 focus:border-indigo-500">{{ old('notes', $purchaseOrder->notes) }}</textarea>
                        @error('notes')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
...
function purchaseOrderForm() {
    return {
        ...
        tax: {{ $purchaseOrder->tax_amount ?? 0 }},
...
@php
    ...
    $breadcrumb = [
        ...
        ['label' => 'Edit', 'url' => route('purchases.edit', $purchaseOrder->id)]
    ];
@endphp
